...
const {
  validateChannelIdRequest,
  validateChannelSettingsUpdateRequest,
...
router.get('/', authCheck, connectionCheck, async (req, res) => {
  ...
  try {
    const updatedChannelsSetting = await updateChannels(userId);
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      await updateChannels(userId);
      const floatplaneChannelSettings = await FloatplaneChannelSetting.findOne({
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!floatplaneChannelSettings)
        return res
          .status(404)
          .json(respondError('No floatplaneChannel settings found'));
  ...
  async (req, res) => {
    ...
    if (!floatplaneChannelSettings)
      return res
        ...
        .json(
          respondError(
            'You are not subscribed to this channel cannot get videos'
          )
        );

    await syncVideos(userId, channelId, req.cookie);

    const videos = await FloatplaneVideo.findAll({
      where: { userId, channelId },
    });
