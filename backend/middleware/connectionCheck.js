...
module.exports.connectionCheck = async (req, res, next) => {
  ...
  try {
    ...
    if (!floatplaneCredential || !floatplaneCredential.cookie)
      return res.status(404).json(respondError('Floatplane not connected'));
    ...
    if (currentTimestamp >= floatplaneCredential.cookieExpires) {
      ...
      return res
        .status(403)
        .json(respondError('Your cookie has expired relog'));
    }
