...
router.post('/register', validateAuthRequest, async (req, res) => {
    ...
    try {
        const user = await User.findOne({ where: { username } });
        if (user) return res.status(401).json(respondError('User already exists'));
        ...
        if (!newUser) return res.status(401).json(respondError('Somthing went wrong while trying to create your user'));
